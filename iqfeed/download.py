...
Bar = namedtuple('IQFeedBar', ['datetime', 'open', 'high', 'low', 'close', 'volume', 'period_volume'])
...
def get_bars(instrument, start_date, end_date, tz, seconds_per_bar,
    ...
    if len(data):
        for line in data.split('\n'):
            ...
            (datetime_str, high, low, open_, close, volume, period_volume, _) = line.split(',')
            if volume.find('.') != -1:
                raise Exception("Float as a volume, strange...: %s" % line)

            log.debug("%s open=%s high=%s low=%s close=%s volume=%s period_volume=%s", datetime_str, high, low, open_, close, volume, period_volume)
            dt = __create_datetime(datetime_str, format_str="%Y-%m-%d %H:%M:%S", timezone=tz)
            (open_, high, low, close, volume, period_volume) = (float(open_), float(high), float(low), float(close), int(volume), int(period_volume))

            bar = Bar(dt, open_, high, low, close, volume, period_volume)
            bars.append(bar)
